...
A refund-claim integrity system for a Shopee-style marketplace. A seller logs in to a dashboard, sees refund claims submitted by buyers for the seller's products, and uses triage output to verify the claims. The system ingests each refund claim (buyer text + photo + product + account context) and produces a **0–100 Risk Score**, a band (Low / Elevated / High), and a plain-English explanation for the seller. Hackathon demo, not production.

## Architecture

```
React frontend  ->  Node/TS backend  ->  OpenAI (vision: Signal 1)
(seller login,      (demo login,          OpenAI (narrator:
 dashboard,          signal runner,        seller explanation)
...
1. Seller signs in with a demo seller login.
2. Seller lands on the claim dashboard.
...
4. Seller reviews claims from buyers of the seller's products, using Risk Score, Risk Band, signal evidence, and explanation.
5. Seller records an advisory action locally. The system never auto-approves or auto-rejects.
...
A final **OpenAI** text call turns (score, band, evidences) into the seller-facing explanation + recommended action.
...
- Frontend: React + TypeScript + Tailwind, built with Vite. Includes demo seller login plus the seller claim dashboard.
- Data: JSON files in `data/`, loaded into memory at startup.

## Hard anti-scope rules (DO NOT)

- ❌ No database, no ORM. JSON in memory only.
- ❌ No production auth or user-management system. Demo seller login is in scope.
...
- The demo seller may own all seeded products unless a separate seller/product ownership map is added deliberately.
